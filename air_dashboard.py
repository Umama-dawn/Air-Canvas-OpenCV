...
print("  ✌️ Index+Middle hold  -> Menu click")
...
def do_whiteboard(ix, iy, states):
    ...
    if states[0] == 1 and states[1] == 0 and states[2] == 0:
        if last_pos is None:
            ...
            if len(undo_stack) > 15: undo_stack.pop(0)
            last_pos = (ix, iy)
...
def do_shapes(ix, iy, states):
    ...
    if states[0] == 1 and states[1] == 0:
        ...
        x1, y1 = last_pos
        c = COLORS[color_idx]; s = BRUSH_SIZES[brush_idx]
        if shape_mode == "Line": cv2.line(canvas, (x1, y1), (ix, iy), c, s)
        elif shape_mode == "Rect": cv2.rectangle(canvas, (x1, y1), (ix, iy), c, s)
        elif shape_mode == "Circle":
            r = int(math.hypot(ix - x1, iy - y1)); cv2.circle(canvas, (x1, y1), r, c, s)
        elif shape_mode == "Star":
            r = int(math.hypot(ix - x1, iy - y1)); pts = []
            for i in range(10):
...
def do_game(frame, ix, iy):
    ...
    if random.random() < 0.025:
        game_balls.append({"x": random.randint(LEFT_BAR_W + 50, W - 50), "y": -20,
                           "vy": random.uniform(2.5, 6), "r": random.randint(18, 32),
                           "color": random.choice([(0,100,255),(0,255,100),(255,100,100),(200,200,0)])})
    new_balls = []
    for b in game_balls:
        b["y"] += b["vy"]
        if math.hypot(b["x"]-ix, b["y"]-iy) < b["r"]+30: game_score += 1; continue
        if b["y"] > H: game_lives -= 1; continue
        new_balls.append(b)
...
def do_mouse(ix, iy, lm):
    ...
    if pinch and not pinch_prev: pyautogui.click()
    pinch_prev = pinch
...
while True:
    ...
    # Mode actions
    if mode == "Whiteboard" and lm: do_whiteboard(ix, iy, states)
    elif mode == "Shapes" and lm: do_shapes(ix, iy, states)
    elif mode == "Game":
        if ix > LEFT_BAR_W and lm: do_game(frame, ix, iy)
        canvas = np.zeros_like(canvas)
    elif mode == "Robotics" and lm:
        do_robotics(frame, lm); canvas = np.zeros_like(canvas)
    elif mode == "Mouse" and lm:
        do_mouse(ix, iy, lm); canvas = np.zeros_like(canvas)

    # Fist clear (2 sec hold)
    if lm and is_fist(lm) and mode in ("Whiteboard", "Shapes"):
        if not fist_active:
            fist_active = True; fist_hold_start = time.time()
        elif time.time() - fist_hold_start > 2.0:
            undo_stack.append(canvas.copy())
            if len(undo_stack) > 15: undo_stack.pop(0)
            canvas = np.zeros_like(canvas)
            fist_active = False
        ...
        cv2.rectangle(frame, (W//2 - 150, 100), (W//2 - 150 + int(300*prog), 130), (0,255,0), -1)
    ...
    now = time.time()
    fps = 0.9*fps + 0.1*(1/max(now-prev_time, 1e-5)); prev_time = now
    ...
    if key == 27: break
    elif key == ord('1'): color_idx = 0
    elif key == ord('2'): color_idx = 1
    elif key == ord('3'): color_idx = 2
    elif key == ord('4'): color_idx = 3
    elif key == ord('5'): color_idx = 4
    elif key == ord('6'): color_idx = 5
    elif key == ord('7'): color_idx = 6
    elif key == ord('8'): color_idx = 7
    elif key == ord('9'): color_idx = 8
    elif key == ord('q'): brush_idx = 0
    elif key == ord('w'): brush_idx = 1
    elif key == ord('e'): brush_idx = 2
    elif key == ord('r'): brush_idx = 3
    elif key == ord('t'): mode = "Whiteboard"
    elif key == ord('y'): mode = "Shapes"
    elif key == ord('u'): mode = "Game"; game_score = 0; game_lives = 5
    elif key == ord('i'): mode = "Robotics"
    elif key == ord('o'): mode = "Mouse"
    elif key == ord('f'): shape_mode = "Circle"
    elif key == ord('g'): shape_mode = "Rect"
    elif key == ord('h'): shape_mode = "Line"
    elif key == ord('j'): shape_mode = "Star"
    elif key == ord('c'): canvas = np.zeros_like(canvas); print("Cleared")
    elif key == ord('z'):
        if undo_stack: canvas = undo_stack.pop()
    elif key == ord('s'):
        fn = f"drawing_{int(time.time())}.png"
        cv2.imwrite(fn, canvas); print(f"Saved: {fn}")
